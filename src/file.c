...
void file_string_free_if_not_arena(String s, Arena *arena) {
    if (munmap(s.ptr, s.count) == -1) assert(false, "Failed to unmap memory");
}
#else
String file_to_string(char *filename, Arena *arena) {
...
// noop
void file_string_free_if_not_arena(String source, Arena *arena) {
}
#endif
